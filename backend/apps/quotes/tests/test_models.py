...
class QuoteRequestModelTest(TestCase):
    ...
    def test_ordering_newest_first(self):
        ...
        # created_at is auto_now_add, so we cannot pass it at creation.
        # Assign distinguishable timestamps after creation (bypassing auto_now_add)
        # so that '-created_at' ordering is deterministic under SQLite.
        base = timezone.now()
        ...
        QuoteRequest.objects.filter(pk=qr1.pk).update(
            created_at=base - timedelta(seconds=10)
        )
        QuoteRequest.objects.filter(pk=qr2.pk).update(created_at=base)
