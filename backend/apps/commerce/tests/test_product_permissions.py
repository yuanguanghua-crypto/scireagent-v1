...
class ProductCompletenessTest(TestCase):
    ...
    def setUp(self):
        self.client = APIClient()
        self.nuc_class = ProductClassFactory(name='Nucleotides & Nucleosides', slug='nucleotides_nucleosides')

    def test_complete_product_has_is_complete_true(self):
        """完整产品 → is_complete=True"""
        product = ProductFactory(
            name='Complete', catalog_no='SC-C-001',
            status='active', product_class=self.nuc_class,
        )
    ...
    def test_incomplete_product_has_is_complete_false(self):
        ...
        product = ProductFactory(
            name='Incomplete', catalog_no='',
            status='draft',
        )
        # draft 产品前台不可见，用 staff 账号读取以验证完整度计算
    ...
    def test_missing_default_sku_is_incomplete(self):
        """有 SKU 但无默认 SKU → 不完整"""
        product = ProductFactory(
            name='NoDefault', catalog_no='SC-ND-001',
            product_class=self.nuc_class,
        )
