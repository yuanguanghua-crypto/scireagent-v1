...
class PubChemEnhancer:
    ...
    def resolve_to_properties(self, identifier: str, namespace: str = 'name') -> dict:
        ...
        同一 (namespace, identifier) 的成功结果缓存 30 天，避免反复查 PubChem。
        not-found / error 不缓存（临时故障不被长期缓存）。
        缓存键对齐五数据源 systematic_name 锚点。

        缓存层是 best-effort 加速层：任何序列化/连接异常都不影响主流程，
        失败时跳过缓存（详见 docs/DATASOURCE_RELIABILITY.md §5）。
        """
        if not self.available or not identifier:
            return {'error': 'pubchempy not available or empty identifier'}

        cache_key = f'pubchem:resolve:{namespace}:{identifier}'
        try:
        ...
        result = self._resolve_to_properties_impl(identifier, namespace)

        # 只缓存 found=True 的结果（含单结果 + 多 candidates）
    ...
    def _resolve_to_properties_impl(self, identifier: str, namespace: str = 'name') -> dict:
    ...
    def _chembl_search(self, identifier: str) -> dict:
        ...
        try:
            ...
            return {
                ...
                "cid": chembl_id,
                "properties": properties,
                "cas_resolved": None,  # ChEMBL 无 CAS 数据；canonical_smiles 已在 properties 中
                "candidates": [],
                "fallback_used": True,
