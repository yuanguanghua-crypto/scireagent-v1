...
class PubChemClient:
    ...
    def lookup_by_cas(self, cas: str) -> Optional[dict]:
        ...
        try:
            resp = request_with_resilience("GET", url, source="pubchem", timeout=self.timeout)
...
class BioProCorpusLookup:
    ...
    复用进程级共享单例（get_shared_retriever，线程安全），避免每次 validate 重建索引，
    且与 ProtocolRecommender 共享同一份索引。详见 docs/DATASOURCE_RELIABILITY.md §7。
    """

    def __init__(self, data_dir: Optional[str] = None):
        # data_dir 参数保留以兼容旧签名；实际复用进程级共享单例
        from apps.knowledge.services.protocol_recommender import get_shared_retriever
        BioProCorpusLookup._retriever = get_shared_retriever()
