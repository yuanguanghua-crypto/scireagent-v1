...
class ResearchGoalDetailSerializer(BaseModelSerializer):
    # ★ 2026-09-23 修 **E1**：`ResearchGoalViewSet.get_serializer_class()` 让 **create/update 走本序列化器**
    #   （`views.py:28-32`），而宽容的 slug 声明此前**只写在 `ResearchGoalListSerializer` 上**（本文件 `:10`）
    #   ⇒ 本类未覆写 ⇒ 由模型 `slug = SlugField(unique=True)`（`models.py:68`）推出 `required=True`
    #   ⇒ **`POST /research-goals/ {name}` 必 400 `"slug: This field is required."`**
    #   ⇒ 而 Goals 页表单里**没有 slug 字段**（也不该有）⇒ **该页新建功能在 UI 上必然失败**。
    #   模型层**本来就会**自动生成 slug（`models.py` 的 `save()`：`base = slugify(self.name) …`，`:105/:151/:246`；
    #   `ProtocolListSerializer` 注释亦写明 "auto-generated by `Protocol.save()`"）⇒ 序列化器强制必填是**多余约束**。
    #   ⇒ 按同文件既有约定逐字补此行（与 `:10` 一致）。
    slug = serializers.SlugField(required=False, allow_blank=True, allow_null=True)
