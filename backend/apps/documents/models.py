...
class DataSourceCache(models.Model):
    ...
    跨路径共享 PubChem/ChEMBL/PubMed/Bioz 的原始 API 响应，避免反复查外部 API。
    与现有 PubChemCache 并存：PubChemCache 仅 SDS 路径用，本表供 AI AUTO MATCH 等路径复用。
    缓存键对齐五数据源 systematic_name 锚点（source + query_namespace + query_key）。
    TTL 按 §5.2 分级（PubChem/ChEMBL 30 天，PubMed 14 天）；过期仍可作 stale 兜底。
    详见 docs/DATASOURCE_RELIABILITY.md §6。
    """
    ...
    source = models.CharField(max_length=20, choices=Source.choices, verbose_name='数据源')
    query_key = models.CharField(max_length=255, verbose_name='查询键',
        help_text='systematic_name / CAS / product_name 等主标识符')
    query_namespace = models.CharField(max_length=20, verbose_name='标识符类型',
    ...
    is_stale = models.BooleanField(default=False, verbose_name='是否过期兜底数据',
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['source', 'query_key'], name='dsc_source_key_idx'),
        ]

    def __str__(self):
        return f'{self.source}:{self.query_namespace}:{self.query_key}'
...
class SdsRevision(models.Model):
    ...
    class DataConfidence(models.TextChoices):
        """数据可信度 — 对应四级降级链来源等级。"""
        HIGH = 'high', '高 (PubChem CID 命中)'
        MEDIUM = 'medium', '中 (SMILES/InChI/名称解析)'
        LOW = 'low', '低 (类别模板)'
        VERY_LOW = 'very_low', '极低 (通用兜底)'
    ...
    data_confidence = models.CharField(
        max_length=10, choices=DataConfidence.choices,
        default=DataConfidence.LOW, verbose_name='数据可信度',
        help_text='high=PubChem CID 命中 / medium=SMILES-InChI-名称解析 / '
                  'low=类别模板 / very_low=通用兜底',
    )
    data_source_detail = models.TextField(
        blank=True, default='', verbose_name='数据来源说明',
        help_text='可读来源说明，如 PubChem CID 12345 / Category template: Oligonucleotides / '
                  'Generic safety notes (no identifier matched)',
    )
