...
from apps.documents.services.workflow import (
    create_coa, approve_coa, generate_sds, approve_sds, withdraw_coa, withdraw_sds,
)
...
class COAWorkflowTest(TestCase):
    ...
    def test_approve_coa_workflow(self):
        ...
        self.assertEqual(approved.status, Coa.Status.PUBLISHED)
    ...
    def test_withdraw_coa_workflow(self):
        """approve (publish) → withdraw 回退到 DRAFT，保留 pdf_path。"""
        coa = create_coa(
            sku_id=self.sku.id,
            lot_number='SC8001-L2026001',
            produced_at=datetime.date(2026, 1, 15),
        )
        approved = approve_coa(coa.id, qc_analyst='Dr. Wang', qa_approval='Dr. Li')
        self.assertEqual(approved.status, Coa.Status.PUBLISHED)
        self.assertIsNotNone(approved.pdf_path)
        withdrawn = withdraw_coa(coa.id)
        self.assertEqual(withdrawn.status, Coa.Status.DRAFT)
        self.assertIsNotNone(withdrawn.pdf_path)
...
class SDSWorkflowTest(TestCase):
    ...
    def test_generate_sds_workflow(self, mock_fetch):
        ...
        self.assertTrue(sds.data_source_detail)
    ...
    def test_generate_sds_no_identifier_degrades_to_generic(self):
        """无 CAS/SMILES/名称 且 无类别 的产品走 L4 通用兜底，不再抛异常，置信度 very_low。"""
        product = ProductFactory(cas='', smiles='', name='', catalog_no='SC9999')
    ...
    def test_withdraw_sds_workflow(self, mock_fetch):
        """approve 设置 current_sds；withdraw 清空指针（PDF 文件保留）。"""
        mock_fetch.return_value = {
            'cid': 702, 'signal_word': 'Warning',
            'pictograms': [], 'hazard_codes': [], 'precaution_codes': [],
            'section_data': {},
...
class COAApiTest(TestCase):
    ...
    def test_approve_coa_api_publishes(self):
        """审批 COA 经 API 写入 status=published（而非 approved）。"""
        self.client.force_authenticate(user=self.admin)
        coa = create_coa(
            sku_id=self.sku.id,
            lot_number='SC8001-L2026001',
...
class SdsApiTest(TestCase):
    ...
    def test_generate_sds_no_cas(self, mock_fetch2, mock_fetch1):
        """无 CAS/SMILES/名称 的产品不再 400，而是走降级链生成（very_low 兜底）。"""
        mock_fetch1.return_value = None
        mock_fetch2.return_value = None
        product = ProductFactory(cas='', smiles='', name='', catalog_no='SC9999')
        self.client.force_authenticate(user=self.admin)
        response = self.client.post('/api/v1/sds-revisions/generate/', {
        ...
        self.assertEqual(response.status_code, 201)
        data = response.json()['data']
        self.assertEqual(data['data_confidence'], SdsRevision.DataConfidence.VERY_LOW)
