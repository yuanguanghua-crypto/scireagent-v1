...
- year 从 pubdate 取前 4 位；doi 优先 elocationid('doi:...') 再 articleids(idtype=doi)；
  authors 取前 20 位用 ', ' 连接；journal 取 source。
- Reference.doi 唯一：回填冲突时跳过 doi，其余字段照常写，不整批失败。
- --limit / --ids 生效。

测试全部打桩 request_with_resilience，不发真实网络请求。
"""
import io
...
CMD = ("apps.bridges.management.commands.backfill_reference_fields."
...
def _resp(entries: dict):
    """entries: pmid -> entry dict。"""
    result = {"uids": list(entries.keys())}
