...
from apps.commerce.models import Product, SKU, ProductClass
...
def _import_product(catalog_no: str, rows: list[dict], report: CSVImportReport) -> None:
    ...
    # Resolve category_l1 slug → ProductClass L1 (classification authority)
    category_l1_slug = first_row.get('category_l1', '').strip()
    product_class = None
    if category_l1_slug:
