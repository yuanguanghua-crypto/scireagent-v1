...
> ⚠️ **推荐部署已改为单机全栈**：阿里云国际站（香港/新加坡）轻量服务器，见 **`docs/DEPLOY_ALIBABA.md`**。
> 本 HF 免费方案作为**零成本备选**保留——但注意 HF 免费版是临时盘 + 会休眠，每次唤醒会重拉 514MB 的 bioprocorpus（必要后台模块），首载较慢；且需搭配 Supabase + Cloudflare 三平台。仅在「完全不想花钱、且能接受慢首载」时采用。
