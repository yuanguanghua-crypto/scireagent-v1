<!-- DEPRECATED: 本文件是项目初始设计阶段（2026-06-11）的数据库架构草案，代表 "LabPro Global" 的早期设计。不代表当前代码库的实际实现，不应作为编码决策的依据。

当前权威文档：CLAUDE.md → 数据模型最小摘要；apps/ 下各 app 的 models.py → 实际字段定义；migrations/ → 实际迁移历史。
