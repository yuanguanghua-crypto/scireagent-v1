<!-- DEPRECATED: 本文件是项目初始设计阶段（2026-06-11）的知识图谱规范草案，代表 "LabPro Global" 的早期设计。不代表当前代码库的实际实现，不应作为编码决策的依据。

当前权威文档：KNOWLEDGE_ASSETS.md → 知识图谱体系（实体、关系、呈现、生成、校验）；KNOWLEDGE_ROADMAP.md → 演进路线图；CLAUDE.md → 核心链路 ResearchGoal → Application → Method → Protocol → Product → SKU。
