<!-- DEPRECATED: 本文件是项目初始设计阶段（2026-06-11）的前端 PRD 草案，代表 "LabPro Global" 的早期需求。不代表当前代码库的实际实现，不应作为编码决策的依据。

当前权威文档：CLAUDE.md → 前端技术栈、目录结构；frontend/src/ → 实际页面与组件；docs/COA_SDS.md → 合规文档前端；docs/AI_AUTO_MATCH.md §6 → AI 面板前端实现。
