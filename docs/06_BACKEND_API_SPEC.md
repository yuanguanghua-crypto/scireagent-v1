<!-- DEPRECATED: 本文件是项目初始设计阶段（2026-06-11）的后端 API 规范草案，代表 "LabPro Global" 的早期设计。不代表当前代码库的实际实现，不应作为编码决策的依据。

当前权威文档：CLAUDE.md → API 端点列表、响应格式（信封格式）；apps/ 下各 app 的 api/v1/urls.py → 实际端点路由；apps/ 下各 app 的 api/v1/views.py → 实际视图实现；docs/AI_AUTO_MATCH.md §7 → AI AUTO MATCH 端点；docs/COA_SDS.md §7 → COA/SDS 端点。
