...
> 适用对象：完全不了解本项目的测试人员（含北京产品测试 `scire03`、休斯顿实验室研究员 `scire02`）。
> 目标：对照本文档，在浏览器中**逐页、逐元素、可遍历**地完成网站所有功能点的手动测试，并规范记录错误与复现步骤。
> 站点标题：`LabPro Global`（对外品牌名），项目代号 `SciReAgent`。
> 文档版本：**v2，2026-07-14 修订**，内容基于**线上服务器实际状态**编写（部署版本 `v2026.07.14` / 代码 `14e2fe3`，站点 `https://scireagent.com`）。
...
1. 先按 **第 1 章** 确定你要测的是「线上站」还是「本地开发」，并准备好对应账号。
...
约定：本文所有路径以「测试基址」为基准。线上站基址 = `https://scireagent.com`；本地基址 = `http://localhost:5173`（端口以你实际拿到的为准）。例如「打开 `/products`」即访问 `<基址>/products`。
...
| 目标 | 基址 | 说明 | 何时用 |
|------|------|------|--------|
| **A. 线上站（推荐，反映真实状态）** | `https://scireagent.com` | 部署版本 `v2026.07.14`，数据真实（8 款产品、真实订单） | 绝大多数功能/回归测试 |
| B. 本地开发 | `http://localhost:5173` | 需自己起前后端（见 1.3），无 Basic Auth | 调试、复现线上 bug、断点 |

> 本手册默认以**线上站 A** 为测试目标。本地 B 仅用于复现疑难问题。
...
npm install        # 仅首次
npm run dev        # 看终端打印的实际 Local 地址（可能 5173/5174/5175）
```
- 本地**无** Basic Auth，直接用网站账号登录即可。
- 验证：打开基址应见首页；打开 `/products` 应见产品列表（非空白）。
...
6. **环境信息**：基址（线上站/本地）、浏览器与版本、Basic Auth 账号。
