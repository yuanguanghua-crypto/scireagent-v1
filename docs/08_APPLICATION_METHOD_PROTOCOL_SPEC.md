<!-- DEPRECATED: 本文件是项目初始设计阶段（2026-06-11）的领域内容中台草案，代表 "LabPro Global" 的早期设计。不代表当前代码库的实际实现，不应作为编码决策的依据。

当前权威文档：KNOWLEDGE_ASSETS.md §2 → 知识实体分类与字段；apps/knowledge/models.py → 实际模型；apps/knowledge/api/v1/ → 实际 API。
