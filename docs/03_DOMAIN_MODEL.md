<!-- DEPRECATED: 本文件是项目初始设计阶段（2026-06-11）的思路草案，代表 "LabPro Global" 的早期领域模型。不代表当前代码库的实际实现，不应作为编码决策的依据。

当前权威文档：KNOWLEDGE_ASSETS.md → 知识实体与桥接表；CLAUDE.md → 数据模型最小摘要；apps/ 下各 app 的 models.py → 实际字段定义。
