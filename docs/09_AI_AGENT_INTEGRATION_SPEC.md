<!-- DEPRECATED: 本文件是项目初始设计阶段（2026-06-11）的 AI/Agent 集成规范草案，代表 "LabPro Global" 的早期设计。不代表当前代码库的实际实现，不应作为编码决策的依据。

当前权威文档：AI_AUTO_MATCH.md → 完整 AI AUTO MATCH 技术文档（5 数据源、降级策略、前端实现）；COA_SDS.md §2 → SDS 数据依赖；KNOWLEDGE_ASSETS.md §6.4-6.5 → AI 推荐与 COA/SDS 自动生成；apps/commerce/services/validators/ → 实际 AI 服务代码。

本文件仅保留作为历史参考。 -->

<!--
DEPRECATED: 本文件是项目初始设计阶段（2026-06-11）的思路草案，不代表当前代码库的实际实现。
不应作为编码决策的依据。不应约束当前开发方向。
当前权威文档：CLAUDE.md（项目根目录）、AI_AUTO_MATCH.md、KNOWLEDGE_ASSETS.md、
COA_SDS.md、KNOWLEDGE_ROADMAP.md。
-->
