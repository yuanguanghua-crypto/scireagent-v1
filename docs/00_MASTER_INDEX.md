<!-- DEPRECATED: 本索引及指向的 12 份文档（01-12）均为项目初始设计阶段（2026-06-11）的产物， 产品名称为 "LabPro Global"（早期代号）。这些文档已严重脱离当前代码库的实际实现， 不再作为编码决策的导航依据。

⚠️ 重要提示：后续所有开发决策，请以以下文档为准：
  - CLAUDE.md（根目录）→ 架构铁律、技术栈、权限、测试命令 — 最高优先级
  - KNOWLEDGE_ASSETS.md → 知识图谱体系（实体、关系、生成、校验）
  - AI_AUTO_MATCH.md → AI AUTO MATCH 功能（四数据源、降级策略）
  - COA_SDS.md → COA/SDS 文档自动生成（数据依赖、工作流）
  - KNOWLEDGE_ROADMAP.md → 当前优化待办（P0/P1/P2 分级）

本文件仅保留作为历史档案索引，记录项目初始设计阶段的文档结构与意图。 -->

<!-- DEPRECATED: 本索引及指向的 12 份文档（01-12 + specifications/ + design/）均为项目初始设计阶段（2026-06-11）的产物，已严重脱离当前代码库的实际实现，不再作为编码决策的导航依据。

若你是 AI 编码助手或研究员，请忽略本文件的阅读顺序和执行顺序建议，直接读取下方 CURRENT DOCUMENTATION 章节列出的权威文档。

若你是研究人员想了解项目最初的设计思路，可以继续阅读下文，但需理解这些文档反映的是初始设想而非当前代码现实。 -->

# Chapter 00 Master Index and Navigation Guide

> ⚠️ 本文件已废弃。当前文档导航请跳转至下方的 CURRENT DOCUMENTATION 章节。（历史档案）

> 本文件是项目初始设计阶段的总索引与执行导航总纲。已降级为历史档案，不再指导当前开发。当前文档体系见下方「CURRENT DOCUMENTATION」章节。
...
## CURRENT DOCUMENTATION（现行文档导航）

以下文档是当前项目开发的**权威依据**，按优先级排列：

| 优先级 | 文档 | 用途 |
|:------:|------|------|
| ⭐⭐⭐ | `../CLAUDE.md` | **唯一入口** — 架构铁律、技术栈锁定、目录结构、权限速查、测试命令、坑列表 |
| ⭐⭐⭐ | `KNOWLEDGE_ASSETS.md` | 知识图谱体系 — 实体分类、关联关系、生成机制、校验机制 |
| ⭐⭐⭐ | `AI_AUTO_MATCH.md` | AI AUTO MATCH 功能 — 四数据源、降级策略、前端实现、已知缺口 |
| ⭐⭐⭐ | `COA_SDS.md` | COA/SDS 文档自动生成 — 数据依赖、工作流、SDS 三级降级链、优化待办 |
| ⭐⭐ | `KNOWLEDGE_ROADMAP.md` | 演进路线图 — P0/P1/P2 优化待办、实施顺序 |
| ⭐ | 本目录 `01`-`12` | **初始设计草案，仅作历史参考，不用于编码决策** |

### 现行文档间关系

```mermaid
flowchart TD
    CLAUDE["CLAUDE.md<br/>架构铁律 + 技术栈 + 权限 + 测试"]
    KA["KNOWLEDGE_ASSETS.md<br/>知识图谱体系"]
    AAM["AI_AUTO_MATCH.md<br/>AI 预填引擎"]
    CSD["COA_SDS.md<br/>合规文档生成"]
    KM["KNOWLEDGE_ROADMAP.md<br/>优化待办"]
...
| 任务类型 | 权威文档路径 |
|---------|------------|
| 产品录入 / AI AUTO MATCH | CLAUDE.md → AI_AUTO_MATCH.md |
| 知识图谱操作 | CLAUDE.md → KNOWLEDGE_ASSETS.md |
| COA / SDS 生成 | CLAUDE.md → COA_SDS.md → AI_AUTO_MATCH.md |
| 系统架构 / 编码决策 | CLAUDE.md（唯一入口） |
| 优化决策 / 迭代规划 | CLAUDE.md → KNOWLEDGE_ROADMAP.md |

> **铁律**：如果当前任务涉及编码或架构决策，**只读上方 ⭐⭐⭐ 文档**。01-12 仅作历史背景，其中的设计假设、流程、约束已经严重脱离当前代码库的实际状态。

---

*历史档案日期：2026-06-11 | 降级标记：2026-06-27*
