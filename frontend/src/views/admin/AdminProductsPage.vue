...
<style scoped>
.admin-page { max-width: var(--content-max-width); margin: 0 auto; padding: 24px; }
...
.product-table tr.row-selected { background: var(--color-info-bg); }
.th-check { width: 40px; }
...
.status-active, .status-published { background: var(--color-success-light); color: var(--color-primary-active); }
.status-draft { background: var(--color-warning-light); color: var(--color-warning); }
.status-archived { background: var(--color-bg); color: var(--color-text-tertiary); }
