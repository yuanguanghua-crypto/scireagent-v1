...
async function save() {
  ...
  try {
    if (editing.value) {
      await http.put(`/research-goals/${editing.value.id}/`, form.value)
    } else {
      await http.post('/research-goals/', form.value)
    }
...
<template>
  <div class="entity-page">
    ...
    <table v-else-if="entities.length" class="entity-table">
      <thead>
        <tr><th>ID</th><th>Name</th><th>Summary</th><th>Status</th><th></th></tr>
      </thead>
      <tbody>
        <tr v-for="e in entities" :key="e.id">
          ...
          <td><span class="status-tag" :class="`status-${e.status}`">{{ e.status || '—' }}</span></td>
          <td><button class="btn btn-ghost btn-sm" @click="openEdit(e)">Edit</button></td>
        </tr>
      </tbody>
    </table>
    <p v-else class="empty-text">No goals yet.</p>

    <!-- Editor dialog -->
    <div v-if="showEditor" class="dialog-overlay" @click.self="showEditor = false">
      <div class="dialog">
        <h3>{{ editing ? 'Edit' : 'New' }} Research Goal</h3>
