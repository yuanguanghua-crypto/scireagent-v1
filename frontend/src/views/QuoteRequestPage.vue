...
<template>
  <div class="rfq-page">
    ...
    <template v-else>
      ...
      <form @submit.prevent="handleSubmit" class="rfq-form" novalidate>
        <!-- Contact Info -->
...
.rfq-input.has-error { border-color: var(--color-danger); }
.rfq-textarea {
  width: 100%;
  padding: 10px 12px;
  border: 1px solid var(--color-border);
...
.rfq-error { font-size: 11px; color: var(--color-danger); }
...
.rfq-remove-btn:hover { background: var(--color-danger-bg); color: var(--color-danger); border-color: var(--color-danger); }
.rfq-add-btn {
  padding: 8px 16px;
