...
/* ── URL category sync ── */
const pendingCategoryFromUrl = ref('')
...
onMounted(() => {
  ...
  if (catFromUrl && typeof catFromUrl === 'string') {
    pendingCategoryFromUrl.value = catFromUrl
  }
})

// When l1List loads, apply pending category from URL
watch(l1List, (list) => {
  if (pendingCategoryFromUrl.value && list.length > 0) {
    selectedL1.value = pendingCategoryFromUrl.value
    expandedL1.value[pendingCategoryFromUrl.value] = true
    pendingCategoryFromUrl.value = ''
    emitFilter()
  }
})
...
<style scoped>
.product-layout {
  width: 100%; margin: 0; padding: 60px 0 0; position: relative;
  /* padding-top: compensates for the 68px fixed PublicNav minus 20px from content-area,
     plus 12px breathing room so category-pills-bar is not flush against nav bottom */ }

/* P1: 深色桥梁已移除，PageBridge 不再使用 */
.product-container {
...
.cat-pill-count { font-size: 11px; color: var(--color-gray-600); }
.cat-pill-clear {
...
/* Wide screen: expand content area for product grids */
@media (min-width: 1400px) {
  .product-container { max-width: 1440px; }
}
@media (min-width: 1800px) {
  .product-container { max-width: 1600px; }
}
@media (min-width: 2560px) {
  .product-container { max-width: 1920px; }
}
</style>
