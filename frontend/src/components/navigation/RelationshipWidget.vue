  ...
  <section class="rw" v-if="hasContent" :aria-label="title">
    ...
    <div class="rw-grid">
      <div v-for="group in groups" :key="group.type" class="rw-group" v-show="group.items?.length">
        <span class="rw-group-label" :style="{ color: TYPE_COLORS[group.type] || 'var(--color-text-secondary)' }">
          {{ group.label }}
        </span>
        <div class="rw-items">
          <button
          ...
          >
            <span class="rw-dot" :style="{ background: TYPE_COLORS[group.type] || 'var(--color-text-secondary)' }"></span>
