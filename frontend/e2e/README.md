...
| 14 | **★ `D16` 稳定失败 `Error: spawnSync ssh EBUSY`**（`helpers/prod-db.cjs`）—— **环境问题，非代码问题**（2026-09-23 重启后出现，重启前 passed） | 实测 node 的 `child_process` 在本会话**无法 spawn 任何外部程序**（`ssh`/`cmd.exe`/`git`/`node`/`bash`/`tar`/`cat` 全部 `EBUSY`，绝对路径同样）；而 **bash 里 `ssh` 正常**。影响面**仅 D16**（只有它用 `prod-db`；`J4` 不受影响）。<br>**临时绕法**：改用 **Bash 直连生产库**取 8 表计数做前后比对（Δ0 结论等价）——命令见 `scireagent-deploy` 技能的「部署后污染只读审查」小节同款 ssh+psql 写法。**根治待查**（线索：`LSBOX_AUDIT_SHMEM=…LiteSandbox…`，但放行沙箱后仍 EBUSY） |
