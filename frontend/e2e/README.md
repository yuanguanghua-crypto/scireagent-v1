...
| 14 | **★ `spawnSync`/`execFileSync` 在默认 `stdio:'pipe'` 下稳定抛 `EBUSY`**（表现为 `D16` 失败 `Error: spawnSync ssh EBUSY`）—— **本机环境问题，非代码问题**（2026-09-23 重启后出现；重启前 passed） | **已定位并已修**（`helpers/prod-db.cjs` 改为把输出**重定向到临时文件**、不建管道 ⇒ **D16 生产 passed 18.6s**）。<br>**可证规则**：`stdio:'ignore'` / `'inherit'` / **文件 fd** ⇒ **一律 OK**；默认 **pipe** ⇒ **一律 EBUSY**（`ssh`/`cmd.exe`/项目内 venv python 都试过）。<br>**已排除**：不是 node 注入的 shim（清空 `NODE_OPTIONS` 仍挂）、不是目标 exe/路径形态（正斜杠/反斜杠/绝对路径都一样）、不是沙箱开关（放行后仍挂）；**Python 的 `subprocess` 与 node 的**异步** `spawn` 都正常** —— 只有"**同步 + 管道**"这个组合被拦。<br>**尚未钉死的下一层**：拦截者是谁（线索 `LSBOX_AUDIT_SHMEM=…LiteSandbox…`），以及为何 `dbSnapshot`（同样用默认管道、目标为**项目内** python）在 Playwright worker 里**能过**而 `prodCounts` 的 `ssh`（**系统**二进制）不能 —— 疑似带"目标/上下文"策略。<br>**注意**：`helpers/db-snapshot.cjs` 的 `execFileSync` **当前可用**，故**未改**（不无谓改动可用路径）；若将来它也报 EBUSY，照 `prod-db.cjs` 的 `runSshSync` 同法处理 |
