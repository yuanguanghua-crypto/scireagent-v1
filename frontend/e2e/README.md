...
| 14 | **★ `spawnSync`/`execFileSync` 抛 `EBUSY`** —— **真因已钉死：子进程的 `stdin` 管道**（`stdio[0]==='pipe'`，也正是这两者的**默认值**） | **已修**（统一走 `helpers/sync-spawn.cjs` 的 `runSync()`：`stdio:['ignore','pipe','pipe']`）。<br>**判别矩阵（逐项实测）**：默认三管道 ⇒ **EBUSY**；**仅 stdin 管道** ⇒ **EBUSY**；仅 stdout 管道 ⇒ **OK（且能正常读回输出）**；仅 stderr 管道 ⇒ **OK**；`['ignore','pipe','pipe']` ⇒ **OK（stdout/stderr 均可捕获）**<br>⇒ **不是"同步读输出"的问题**（仅 stdout 时读回完全正常），而是**建 stdin 管道**被拦。旁证：**异步 `spawn`** 用默认管道可以、**Python 的 `subprocess`** 也正常 ⇒ 只有"**同步 + stdin 管道**"这一组合。<br>**已排除**（均实测）：node 注入的 shim（清空 `NODE_OPTIONS` 仍挂）、目标 exe/路径形态、沙箱开关（放行后仍挂）。<br>**拦截者已查明（2026-09-23）**：环境变量 `LSBOX_AUDIT_SHMEM=Local\LiteSandbox_Audit_**<PID>**_<序号>` 里
**第 3 段就是拦截者 PID** ⇒ `tasklist /FI "PID eq <PID>"` ⇒ **`sandbox-cli.exe`**（同机另有
**`codex-windows-sandbox-service.exe`（Windows 服务）**、`sandbox-center.exe`、`sandbox-cli-gc.exe`；
`SANDBOX_CENTER_IPC_ADDRESS=\\.\pipe\WorkBuddy_SandboxCenter_…` 坐实归属）
⇒ 这套 **WorkBuddy `SandboxCenter` ↔ `LSBox(LiteSandbox)` ↔ Codex 系沙箱**包装进程创建、发布审计共享内存，
**正是它拦掉 stdin 管道**。⚠️ 它是**宿主的安全控制** ⇒ **不要停用/绕开**；用上面的 `stdio` 修法即可零副作用绕过。
（旁证：`PLAYWRIGHT_MCP_NO_SANDBOX=1` / `AGENT_BROWSER_ARGS=--no-sandbox` ⇒ Chromium 自带沙箱关闭，故浏览器不受影响。）
⚠️ **现象是动态/间歇的**（08:1x 全过 → 08:45 起本地 venv python 也中招）⇒ 全 e2e 的同步 spawn **统一收口**到 `sync-spawn.cjs`（现仅此一处实现） |
