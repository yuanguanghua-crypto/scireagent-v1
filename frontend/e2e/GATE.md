...
### 9.7 ✅ 结论：采用方案①，**判据③ 已达成**（2026-09-24）

**用户拍板采用 ①**（批间刷新环境），并于同日连跑三轮复核（**每轮流程**：停旧 Vite+Django → 重启 → 就绪自检三条 → **预热 Vite 10 个模块** → 跑 470 例 `--retries=0`）：

| 轮 | 退出码 | passed | failed | **加载类超时** | 耗时 |
|---|---|---|---|---|---|
| R1 | **0** | 469 (+1 条件性 skip) | 0 | **0** | 13.7m |
| R2 | **0** | 469 (+1 条件性 skip) | 0 | **0** | 13.9m |
| R3 | **0** | 469 (+1 条件性 skip) | 0 | **0** | 14.8m |

⇒ **连续 3 批 0 新缺陷、退出码全 0 ⇒ 判据③ 达成。** 且耗时稳定在 ~14m（对照前一晚 18.3m → 21.9m 的递增），
**同时证实**昨晚那 6 条加载类超时是**服务端状态累积**（Vite transform 缓存增长 / Django+SQLite 夹具反复增删膨胀）所致，**不是内存也不是 CPU**。

> ⚠️ 顺带更正一处**曾经写错的环境归因**：此前多次称"本机 16GB + CPU 80% 上限"——实测（2026-09-24）为 **63.8 GB 内存 / i7-13700KF / `PROCTHROTTLEMAX` = 100%**。
> 16GB 是**另一台机器**（MSI Cyborg 15 笔记本）的约束；80% 上限**当时也已不在生效**。**凡性能/资源类归因，必须先实测当前机器规格。**
